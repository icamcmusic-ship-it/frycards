...
export function cardRules(def: CardDef): string {
  ...
  if (def.ultimate) bits.push(`Ultimate turn ${def.ultimate.unlockTurn}+ (${def.ultimate.threshold}+): ${describeEffect(def.ultimate.effect)}`);
  return bits.join(' · ');
}
...
export function CardFace({
  def, small, dimmed, highlight, onClick, footer, badge, count,
}: {
  def: CardDef; small?: boolean; dimmed?: boolean; highlight?: boolean;
  onClick?: () => void; footer?: React.ReactNode; badge?: string; count?: number;
}) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      {def.image && (
        <div className={cn('ink-border-sm mx-1 mt-0.5 overflow-hidden relative', small ? 'h-[52px]' : 'h-[66px]')}>
          <img src={def.image} className="w-full h-full object-cover" draggable={false} loading="lazy" />
          {def.rarity && (
            <span
              className={cn(
