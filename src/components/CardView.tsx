...
// Keyword Glossary tooltips (QOL): hover a keyword chip to read the rule.
const KEYWORD_GLOSSARY: Record<string, string> = {
  Armor: 'Absorbs incoming damage. Base Armor is destroyed once bypassed; Armor from attached Items keeps applying. If Armor soaks the whole hit, no damage is dealt (no Wither/Reap triggers).',
  Blitz: 'May attack the turn it is deployed (no summoning sickness).',
  Boost: 'Adds +X to your resource roll.',
  Brittle: 'Takes double damage from all sources.',
  Burden: 'Attacking with the equipped Unit costs X extra generic resources.',
  Command: 'Leader ability: pay X to instantly ready a friendly Unit for another attack.',
  Decay: 'Harmful Charm: attaches to the enemy player.',
  Detonate: 'When this Charm expires, deal X damage to all enemy Units.',
  Echo: 'Chaos Events: on a d6 roll of 5-6, the Event resolves a second time on a random target.',
  Feedback: 'When targeted by an enemy card, roll a d6. On 4-6 the play is negated and fully refunded; Events return to hand.',
  Fix: 'Automatically allocates 1 point of your roll to the named element.',
  Glitch: 'Combat damage disables the target\'s keywords until Cleanup.',
  Graveborn: 'May be deployed from your graveyard during your Action Phase. It is removed from the game when it dies again.',
  Guard: 'Ready Guard Units must be attacked/targeted first. Guard suppresses Lurk.',
  Lurk: 'Cannot be targeted by enemy cards (suppressed while this Unit has Guard).',
  Meltdown: 'Destroy a target Item; its host takes Flame damage equal to the Item\'s cost.',
  Modularity: 'The host Unit can hold X additional Items.',
  Overclock: 'Gain X resources now; X is deducted from your next roll only — the debt never rolls over further.',
  Overdrive: 'May attack twice per turn, then takes 2 damage at Cleanup if it did.',
  Pierce: 'Excess combat damage beyond a blocker carries over to the enemy Leader.',
  Pure: 'Bonus effect if your entire roll this turn went into a single element.',
  Purge: 'Strip all Items, statuses, Wither and temporary buffs from a target.',
  Reap: 'Destroying an enemy Unit in combat heals your Leader for its base attack.',
  Siphon: 'Your Leader heals half the damage this card deals to ENEMY cards or the enemy Leader (rounded up).',
  Sustain: 'Heals X damage at the start of your turn.',
  Symmetric: 'This Location\'s effect applies equally to both players.',
  Ward: 'Enemy cards targeting this Unit cost X extra generic resources.',
  Wildcast: 'Splashes onto X random enemy Units. Targets are always unique.',
  Wither: 'Combat damage permanently reduces the target\'s ATK and max health by X. A Unit reduced to 0 max health is destroyed.',
};
...
function displayArmor(card: GameCard): number {
  const itemArmor = (card.attachedItems || []).reduce((s, it) => {
    const kw = (it.keywords || []).find((k) => k.split(' ')[0] === 'Armor');
    return s + (kw && !it.glitched ? parseInt(kw.split(' ')[1], 10) || 0 : 0);
  }, 0);
  return (card.glitched ? 0 : card.armor) + itemArmor;
}
function healthRemaining(card: GameCard): number {
  const max = Math.max(0, (card.health || 0) + card.tempHp - card.witherHp);
  return Math.max(0, max - card.damageTaken) + Math.max(0, bonusHp(card) - card.bonusDamage);
}
    ...
    >
      ...
      <div className="relative w-full aspect-[4/3] bg-[#2C3E50] ink-border-sm overflow-hidden mx-auto" style={{ width: 'calc(100% - 8px)' }}>
        ...
        <div className="absolute top-0.5 right-0.5 flex flex-col gap-0.5 items-end">
          {displayArmor(card) > 0 && <span className="flex items-center gap-0.5 px-1 bg-[#1A1A1A] text-[#F7F7F7] text-[8px] font-bold"><Shield className="w-2.5 h-2.5" />{displayArmor(card)}</span>}
        ...
        {card.keywords && card.keywords.length > 0 && (
          <div className="flex flex-wrap gap-0.5 mt-0.5">
            {card.keywords.map((kw) => (
              <span key={kw} title={KEYWORD_GLOSSARY[kw.split(' ')[0]] || card.text}
                className="text-[7px] font-bold px-0.5 bg-[#FFD54F] text-[#1A1A1A] cursor-help">{kw}</span>
            ))}
          </div>
        )}
      </div>
