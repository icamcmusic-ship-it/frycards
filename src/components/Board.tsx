...
function needsTarget(card: GameCard): boolean {
  ...
  if (
    card.type === 'Event' &&
    card.effect &&
    (card.effect.target === 'unit' || card.effect.target === 'friendly')
  )
    return true;
  return false;
}
...
function ActionToast({ log }: { log: string[] }) {
  ...
  return (
    <div
      key={toast.id}
      className="absolute left-1/2 top-[38%] -translate-x-1/2 z-40 pointer-events-none action-toast"
    >
      <div
        className={cn(
...
export function Board({ gameState, dispatch }: BoardProps) {
  ...
  const opponent =
    gameState.players[
      viewingPlayerId === gameState.player1Id ? gameState.player2Id : gameState.player1Id
    ];
  ...
  const [viewingGraveyardPlayerId, setViewingGraveyardPlayerId] = React.useState<string | null>(
    null,
  );

  const pendingCard = pendingCardId
    ? viewer.hand.find((c) => c.instanceId === pendingCardId) ||
      viewer.board.find((c) => c.instanceId === pendingCardId) ||
      (viewer.leader.instanceId === pendingCardId ? viewer.leader : null) ||
      viewer.locations.find((c) => c.instanceId === pendingCardId)
    : null;
  const isPendingCardOnBoard =
    pendingCard && viewer.hand.findIndex((c) => c.instanceId === pendingCard.instanceId) === -1;
  ...
  const canCommand =
    ...
    isViewingActive &&
    commandCost > 0 &&
    canAfford({ Generic: commandCost }, viewer.resources) &&
    viewer.board.length > 0;
  ...
  const handleUnitClick = (card: GameCard, isOpponent: boolean) => {
    ...
    if (pendingCard) {
      ...
      const wantsFriendly =
        pendingCard.type === 'Item' || pendingCard.effect?.target === 'friendly';
      if (wantsFriendly && !isOpponent && card.type === 'Unit') {
        dispatch({
          type: actionType,
          instanceId: pendingCard.instanceId,
      ...
      if (
        !wantsFriendly &&
        (pendingCard.type === 'Event' || isPendingCardOnBoard) &&
        card.type === 'Leader'
      ) {
        dispatch({
          type: actionType,
    ...
    if (phase === 'COMBAT_DECLARE' && isViewingActive) {
      if (!isOpponent) {
          ...
          (card.summoningSickness ||
            (card.exhausted &&
              card.attacksThisTurn >=
                (card.keywords?.some((k) => k.startsWith('Overdrive')) ? 2 : 1)))
        )
          return;
        ...
        dispatch({
          ...
          targetId: opponent.leader.instanceId,
        });
        ...
        if (already)
          dispatch({ type: 'TOGGLE_ATTACKER', instanceId: src, targetId: already.targetId });
  ...
  const targetingFriendly =
    pendingCard && (pendingCard.type === 'Item' || pendingCard.effect?.target === 'friendly');
  ...
  const blockAssignedTo = (attackerId: string) =>
    combat?.blockers.find((b) => b.attackerId === attackerId)?.blockerId;
  ...
  const hasCastableGraveborn =
    ...
    viewer.graveyard.some(
      (c) =>
        c.type === 'Unit' &&
        c.keywords?.some((k) => k.split(' ')[0] === 'Graveborn') &&
        canAfford(c.cost, viewer.resources),
    );
    ...
    <div className="flex flex-col h-screen bg-[#F7F7F7] text-[#1A1A1A] overflow-hidden">
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-3 items-start">
            <CardView
              ...
              targetable={
                !!targetingEnemy && (pendingCard?.type === 'Event' || !!isPendingCardOnBoard)
              }
            />
            <div className="flex gap-1.5 flex-wrap">
              {opponent.board.map((u) => {
                const isAttackTarget =
                  phase === 'COMBAT_DECLARE' &&
                ...
                return (
                  <div key={u.instanceId} className="relative">
                    ...
                    />
                    {isAttackTarget && (
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-[9px] font-black bg-[#E53935] text-[#F7F7F7] px-1 ink-border-sm pointer-events-none">
                        ⚔ TARGET
                      </div>
                    )}
                  </div>
          ...
          <div className="text-right shrink-0 bg-[#F7F7F7] ink-border-md shadow-hard-black-sm px-4 py-2">
            ...
            <div className="text-[#2C3E50] text-[10px] font-bold mt-1">
              ...
              <button
                onClick={() => setViewingGraveyardPlayerId(opponent.id)}
                className="underline text-[#E53935] font-black hover:text-[#1A1A1A] cursor-pointer"
              >
                GY {opponent.graveyard.length}
              </button>
            </div>
            <div className="text-[#2C3E50] text-[10px] font-bold">
              LOCATIONS SET: {opponent.locations.length}
            </div>
      ...
      <div className="min-h-32 bg-[#FFD54F] border-y-4 border-[#1A1A1A] flex items-center justify-between px-6 py-2 gap-4 relative">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="text-[9px] font-black heading-font bg-[#1A1A1A] text-[#FFD54F] px-1.5 py-1">
                ...
                <span className="text-[#F7F7F7]">
                  {gameState.players[gameState.activeLocationOwnerId!]?.name} CONTROLS
                </span>
              </div>
            </div>
          ) : (
            <div className="text-[10px] font-black heading-font text-[#1A1A1A] opacity-60">
              NO ACTIVE LOCATION
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-center gap-2 flex-1 z-10">
          ...
          {gameState.log.length > 0 && (
            <div className="flex flex-col gap-1 w-full max-w-md items-center pointer-events-none">
              {gameState.log.slice(-3).map((logMsg, i, arr) => (
                <div
                  key={`${gameState.log.length - arr.length + i}`}
                  className={cn(
          ...
          {pendingCard && (
            <div className="text-[11px] font-bold bg-[#E53935] text-[#F7F7F7] px-2 py-0.5 ink-border-sm">
              SELECT A {targetingFriendly ? 'FRIENDLY' : 'ENEMY'} UNIT FOR{' '}
              {pendingCard.name.toUpperCase()} ·{' '}
              <button className="underline" onClick={() => setPendingCardId(null)}>
                CANCEL
              </button>
            </div>
          )}
          {commandMode && (
            <div className="text-[11px] font-bold bg-[#E53935] text-[#F7F7F7] px-2 py-0.5 ink-border-sm">
              ...
              <button className="underline" onClick={() => setCommandMode(false)}>
                CANCEL
              </button>
            </div>
          )}
              ...
              <button
                ...
                title={
                  viewer.hand.length > 7
                    ? `You will discard ${viewer.hand.length - 7} card(s) down to the 7-card hand limit.`
                    : undefined
                }
              >
          ...
          {phase === 'COMBAT_DECLARE' && isViewingActive && (
            <div className="flex gap-3 items-center">
              <span className="text-[10px] font-bold max-w-52">
                Click your Units/Leader to attack the enemy Leader. Then click an enemy Unit to
                redirect your last attacker at it.
              </span>
              <button
          ...
          {phase === 'COMBAT_BLOCK' && isViewingDefender && (
            <div className="flex gap-3 items-center">
              <span className="text-[10px] font-bold max-w-52">
                Select an attacker, then a blocker. Multiple blockers may gang up on one attacker.
              </span>
              <button
        ...
        <div className="w-44 shrink-0 flex justify-end gap-1 z-10">
          {viewer.locations.map((loc, i) => (
            <CardView key={i} card={loc} faceDown compact className="scale-75 origin-right" />
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col p-3 relative min-h-0 bg-[#F7F7F7]">
        <div className="flex justify-between items-start">
              ...
              {viewer.board.map((u) => {
                ...
                return (
                  <div key={u.instanceId} className="relative">
                    ...
                    />
                    {blockedBy && (
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-[9px] font-black bg-[#2C3E50] text-[#F7F7F7] px-1 ink-border-sm pointer-events-none">
                        BLOCKING
                      </div>
                    )}
                  </div>
          ...
          <div className="text-right shrink-0 bg-[#1A1A1A] text-[#F7F7F7] ink-border-md shadow-hard-black-sm px-4 py-2">
            <div className="heading-font text-sm text-[#FFD54F]">
              {viewer.name} {viewer.id === activePlayerId ? '· ACTIVE' : ''}
            </div>
            ...
            <div className="text-[10px] font-bold mt-1 text-[#F7F7F7]/70">
              ...
              <button
                onClick={() => setViewingGraveyardPlayerId(viewer.id)}
                className={cn(
                  'underline text-[#FFD54F] font-black hover:text-[#F7F7F7] cursor-pointer',
                  hasCastableGraveborn && 'animate-pulse bg-[#E53935] text-[#F7F7F7] px-1',
                )}
              >
                GY {viewer.graveyard.length}
                {hasCastableGraveborn ? ' ⭐' : ''}
              </button>
            </div>
            ...
            <div className="mt-2 flex gap-1 justify-end flex-wrap max-w-xs">
              {Object.entries(viewer.resources).map(
                ([el, amt]) =>
                  amt > 0 && (
                    <div
                      key={el}
                      className="px-1.5 py-0.5 bg-[#FFD54F] text-[#1A1A1A] text-[10px] font-black heading-font"
                    >
                      {amt} {el.toUpperCase()}
                    </div>
                  ),
              )}
              {Object.values(viewer.resources).every((v) => !v) && (
                <div className="text-[10px] font-bold text-[#F7F7F7]/50">NO RESOURCES</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto flex justify-center gap-2 overflow-x-auto pb-1 pt-3">
          {viewer.hand.map((card) => {
            const isPlayable =
              phase === 'ACTION' && isViewingActive && canAfford(card.cost, viewer.resources);
            return (
              <CardView
                key={card.instanceId}
                card={card}
          ...
          {viewer.hand.length === 0 && (
            <div className="text-[#2C3E50] font-bold text-sm self-center">EMPTY HAND</div>
          )}
        </div>
      </div>
      ...
      {(() => {
        const gyPlayer = viewingGraveyardPlayerId
          ? gameState.players[viewingGraveyardPlayerId]
          : null;
          ...
          <div className="absolute inset-0 bg-[#1A1A1A]/90 flex items-center justify-center z-50 p-4">
            <div className="bg-[#F7F7F7] text-[#1A1A1A] p-6 ink-border-md shadow-hard-yellow text-center max-w-4xl w-full flex flex-col max-h-[85vh]">
              <div className="flex justify-between items-center mb-4 border-b-4 border-[#1A1A1A] pb-2">
                ...
                <button
                  onClick={() => setViewingGraveyardPlayerId(null)}
                  className="btn-pop bg-[#E53935] text-[#F7F7F7] px-3 py-1 font-black ink-border-sm heading-font"
                >
                  Close [X]
                </button>
              </div>
              <div className="flex-1 overflow-y-auto flex gap-4 flex-wrap justify-center p-2">
                {gyPlayer.graveyard.map((card, idx) => {
                  ...
                  return (
                    <div
                      key={card.instanceId + '_' + idx}
                      className="flex flex-col items-center gap-2 bg-[#F7F7F7] p-2 ink-border-sm shadow-hard-black-xs relative"
                    >
