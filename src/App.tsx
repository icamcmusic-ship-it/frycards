...
function MulliganModal({
  ...
  const toggle = (id: string) =>
    setBottoming((b) =>
      b.includes(id) ? b.filter((x) => x !== id) : b.length < needBottom ? [...b, id] : b,
    );
    ...
    <div className="absolute inset-0 bg-[#1A1A1A]/90 flex items-center justify-center z-50 p-4">
      <div className="bg-[#F7F7F7] text-[#1A1A1A] p-6 ink-border-md shadow-hard-yellow text-center max-w-4xl w-full">
        ...
        <div className="flex justify-center gap-2 flex-wrap mb-5">
          {human.hand.map((c: GameCard) => (
            <CardWithSelect
              key={c.instanceId}
              card={c}
              selected={bottoming.includes(c.instanceId)}
              onClick={() => needBottom > 0 && toggle(c.instanceId)}
              selectable={needBottom > 0}
            />
          ))}
        </div>
        <div className="flex gap-4 justify-center">
          <button
            onClick={keep}
            disabled={needBottom > 0 && bottoming.length !== needBottom}
          ...
          <button
            onClick={() => {
              dispatch({ type: 'MULLIGAN', playerId: human.id });
              setBottoming([]);
            }}
    ...
    >
      <div className={cn(selected && 'outline outline-4 outline-[#E53935]')}>
        <div className="w-24 h-[134px] overflow-hidden bg-[#F7F7F7] text-[#1A1A1A] ink-border-sm shadow-hard-black-xs flex flex-col">
          <div className="text-[9px] heading-font px-1 py-0.5 truncate bg-[#1A1A1A] text-[#FFD54F]">
            {card.name}
          </div>
          {card.image && <img src={card.image} className="w-full aspect-[4/3] object-cover" />}
          <div className="text-[8px] font-mono font-bold px-1 text-[#2C3E50] mt-auto uppercase">
...
function RollModal({
  ...
  useEffect(() => {
    setAllocs({});
  }, [state.pendingRoll, state.turnNumber]);
      ...
      <div className="bg-[#F7F7F7] text-[#1A1A1A] p-8 ink-border-md shadow-hard-yellow text-center max-w-md w-full relative overflow-hidden">
        ...
        {state.phase === 'ROLL' ? (
          ...
          <div className="relative">
            ...
            <p className="text-[#2C3E50] mb-5 text-sm font-bold">
              Distribute {remaining} more into your elements.
            </p>
            <div className="flex flex-col gap-3 mb-6">
              {elements.map((el) => (
                <div
                  key={el}
                  className="flex items-center justify-between bg-[#F7F7F7] p-3 ink-border-sm shadow-hard-black-xs"
                >
                  <span className="heading-font text-sm">{el}</span>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => change(el, -1)}
                      className="btn-pop w-8 h-8 bg-[#1A1A1A] text-[#FFD54F] font-black ink-border-sm"
                    >
                      -
                    </button>
                    ...
                    <button
                      onClick={() => change(el, 1)}
                      className="btn-pop w-8 h-8 bg-[#FFD54F] text-[#1A1A1A] font-black ink-border-sm"
                    >
                      +
                    </button>
                    <button
                      onClick={() =>
                        remaining > 0 &&
                        setAllocs({ ...allocs, [el]: (allocs[el] || 0) + remaining })
                      }
            ...
            <button
              onClick={() => dispatch({ type: 'ALLOCATE_RESOURCES', allocations: allocs })}
              disabled={remaining !== 0}
  ...
  return (
    <div className="absolute inset-0 bg-[#FFD54F] flex items-center justify-center z-50 overflow-hidden">
      ...
      <div className="text-center relative bg-[#F7F7F7] ink-border-lg shadow-hard-black-lg p-12">
        <h1 className="text-6xl heading-font text-[#1A1A1A] mb-4">
        ...
        <div className="flex gap-3 justify-center">
          <button
            onClick={onReplay}
            className="btn-pop px-8 py-3 bg-[#1A1A1A] text-[#FFD54F] heading-font ink-border-md shadow-hard-yellow"
          >
            Back to Menu
          </button>
        </div>
              ...
              {validDecks.map((d: DeckRow) => {
                ...
                return (
                  <button
                    key={d.id}
                    onClick={() =>
                      onStart({ leaderId: d.leader_id, customDeck: d.card_ids, deckName: d.name })
                    }
                  ...
                  >
                    ...
                    <div className="p-3 pt-1">
                      ...
                      <div className="text-[10px] font-bold text-[#2C3E50] mt-0.5 uppercase">
                        {leader?.elements.join(' / ')}
                      </div>
                    </div>
                  </button>
                );
        ...
        <div className="flex flex-wrap gap-4">
          {leaders.map((id) => {
            ...
            return (
              <button
                key={id}
                onClick={() => onStart({ leaderId: id, deckName: `${l.name} (Prebuilt)` })}
                className="btn-pop w-56 overflow-hidden bg-[#F7F7F7] ink-border-md shadow-hard-black hover:-translate-y-1 transition-all text-left"
              >
                <div className="flex justify-between items-center px-2 py-1 bg-[#1A1A1A]">
                  ...
                  <span className="text-[9px] font-mono font-bold text-[#F7F7F7]">
                    {l.health} HP
                  </span>
                </div>
                {l.image && (
        ...
        <button
          onClick={() => {
            if (
              ...
              window.confirm('Concede this match and return to the menu?')
            )
              onExit();
          }}
