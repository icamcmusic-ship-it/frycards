...
export const ENTRIES: ChangelogEntry[] = [
  {
    ...
    date: 'July 2026',
    items: [
      'RULES CHANGE: CHARMS ARE NOW ITEMS. Every Charm you own is an Item — same card, same cost, same stats, same keywords, new type line. Nothing was reprinted.',
      'Items come in three flavours. A CHARM still dies with the unit it is bonded to — but it can now also be cast on YOU instead, restoring Vitality equal to its whole bond. That makes it the only Item you can play with an empty board.',
      'A WEAPON survives the unit it was bonded to and re-bonds to another for its re-bond cost — that is the old "Worn". A TOOL is a Weapon that also WEAKENS a target enemy unit as it lands, permanently; it carries a slightly smaller bond to pay for it.',
      'The CPU plays both new lines — it will cast a Charm on itself when it is actually hurt, and it aims Tools at your best body.',
      'PLAYER SHOWCASE is now PLAYERS SHOWCASE 2026, and the screen finally tells you what it wants: 100 cards before the set gets its own booster pack and its Ultra-Rare community vote. There is a progress bar.',
      'Card previews on the submissions screen are full size now — the same card you would own — and every one of them opens in the 3D inspector when you click it. Your own submissions are shown as cards instead of as a list.',
      'ART SIZES ARE PRINTED. Standard frame: 4:3 landscape (about 1600x1200). Full art: 5:7 portrait (about 1500x2100). Video Mythic: the same 5:7 portrait, short and silent so it loops cleanly. Submit at those and nothing gets cropped.',
      'Mechanics are still generated by the game from your card — but Fry can now overwrite any of them (cost, Might/Grit, keywords, subtype, rules text) before he prints it, when the generated card does not suit the art. Anything he leaves alone stays generated.',
      'FIX: 16 cards were priced one essence higher in the shop/deck-building backend than they actually cost in a match, because a balance change shipped to the game and never reached the database. Everything now agrees.',
      'Backend hardening: seven card-submission endpoints were reachable (though not usable) without logging in, one database function had an unpinned search path, and the submissions table was missing two indexes. All fixed.',
    ],
  },
  {
