...
export function validateDeckList(
  ...
  if (cardIds.length !== DECK_SIZE) issues.push({ text: `Deck must be exactly ${DECK_SIZE} cards (currently ${cardIds.length}).` });
  ...
  for (const id of cardIds) {
    const c = db.get(id);
    if (!c) { issues.push({ text: `Unknown card: ${id}` }); continue; }
  ...
  if (locations < 2) issues.push({ text: `Deck needs at least 2 Location cards (has ${locations}).` });
  ...
  if (collection) {
    ...
    for (const [id, n] of byId) {
      ...
      if (n > have) {
        const c = db.get(id);
        issues.push({ text: `You only own ${have} cop${have === 1 ? 'y' : 'ies'} of ${c?.name || id}.` });
      }
    }
...
export function DeckBuilderScreen({ onBack, allCards }: { onBack: () => void; allCards: CardTemplate[] }) {
  ...
  if (editing) {
    return (
      <DeckEditor
        ...
        allCards={allCards}
        onDone={() => { setEditing(null); refreshDecks(); }}
      />
    );
  }
    ...
    <div className="w-full min-h-screen bg-[#F7F7F7] text-[#1A1A1A]">
      ...
      <div className="p-5 max-w-5xl mx-auto">
        <PopButton color="red" onClick={() => setEditing('new')} className="mb-5">
          <span className="flex items-center gap-1"><Plus className="w-4 h-4" /> FORGE NEW DECK</span>
        </PopButton>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {decks.map((d) => {
            const leader = allCards.find((c) => c.id === d.leader_id);
            return (
              <div key={d.id} className="bg-[#F7F7F7] ink-border-md shadow-hard-black-sm overflow-hidden">
                <div className="flex justify-between items-center px-3 py-1.5 bg-[#1A1A1A]">
                  ...
                  {d.is_valid
                    ? <span className="text-[9px] font-black text-[#F7F7F7] bg-[#2C3E50] px-1 flex items-center gap-0.5"><Check className="w-3 h-3" />LEGAL</span>
                    : <span className="text-[9px] font-black text-[#1A1A1A] bg-[#FFD54F] px-1 flex items-center gap-0.5"><AlertTriangle className="w-3 h-3" />INCOMPLETE</span>}
                </div>
                {leader?.image && (
...
// Editor
// ---------------------------------------------------------------------------
function DeckEditor({ deck, allCards, onDone }: { deck: DeckRow | null; allCards: CardTemplate[]; onDone: () => void }) {
  ...
  const pool = allCards.filter((c) => {
    ...
    if (leader && !c.elements.every((e) => e === 'Generic' || leader.elements.includes(e))) return false;
          ...
          {ownedLeaders.map((l) => (
            <button key={l.id} onClick={() => setLeaderId(l.id)}
              ...
              <div className="p-3 pt-1">
                ...
                <div className="text-[10px] font-bold text-[#2C3E50] mt-1 uppercase">{l.elements.join(' / ')}</div>
              </div>
            </button>
          ))}
          {ownedLeaders.length === 0 && (
            <div className="text-center font-bold text-[#2C3E50] py-14">You don't own any Leaders yet. Open packs to find one!</div>
          )}
        </div>
      </div>
    );
    ...
    <div className="w-full h-screen flex flex-col bg-[#F7F7F7] text-[#1A1A1A]">
      ...
      <div className="flex items-center justify-between gap-3 bg-[#1A1A1A] px-4 py-2.5 shrink-0">
        ...
        <div className="flex items-center gap-3 shrink-0">
          <span className={cn('heading-font text-sm px-2 py-1 ink-border-sm', cardIds.length === DECK_SIZE ? 'bg-[#FFD54F] text-[#1A1A1A]' : 'bg-[#E53935] text-[#F7F7F7]')}>
      ...
      {(issues.length > 0 || saveError) && (
        <div className="bg-[#FFD54F] border-b-4 border-[#1A1A1A] px-4 py-1.5 text-[10px] font-bold flex flex-wrap gap-x-4 gap-y-0.5 shrink-0">
          ...
          {issues.slice(0, 4).map((i) => <span key={i.text}>⚠ {i.text}</span>)}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          <div className="flex gap-2 items-center p-3 pb-2 shrink-0">
            <input className={cn(select, 'w-44 placeholder:text-[#2C3E50]/50')} placeholder="Search…"
              value={search} onChange={(e) => setSearch(e.target.value)} />
            <select className={select} value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}>
              {['All', 'Unit', 'Event', 'Item', 'Charm', 'Location'].map((t) => <option key={t}>{t}</option>)}
            </select>
            <span className="text-[10px] font-bold text-[#2C3E50] ml-auto">
              POOL: OWNED CARDS IN {leader.elements.filter((e) => e !== 'Generic').join('/').toUpperCase()}
            </span>
          </div>
          <div className="flex-1 overflow-y-auto p-3 pt-0 flex flex-wrap gap-2.5 content-start">
            {pool.map((c) => {
              const inDeck = countOf(c.id);
              const maxAddable = Math.min(MAX_COPIES - nameCount(c), (ownedQty.get(c.id) || 0) - inDeck);
              return (
                <StaticCard
                  key={c.id}
                  card={c}
            ...
            {pool.length === 0 && <div className="w-full text-center font-bold text-[#2C3E50] py-10">No owned cards match.</div>}
          </div>
        </div>
        ...
        <div className="w-72 shrink-0 border-l-4 border-[#1A1A1A] bg-[#2C3E50] flex flex-col">
          <div className="px-3 py-2 heading-font text-xs text-[#FFD54F] shrink-0">
            DECK LIST · {grouped.reduce((s, g) => s + (g.card.type === 'Location' ? g.n : 0), 0)} LOCATIONS
          </div>
          <div className="flex-1 overflow-y-auto px-2 pb-3 flex flex-col gap-1">
            {grouped.map(({ card, n }) => (
              <button key={card.id} onClick={() => removeCard(card.id)}
                ...
                <span className={cn('text-[8px] font-black px-1 shrink-0', RARITY_CHIP[card.rarity || 'Common'])}>
            ...
            {grouped.length === 0 && (
              <div className="text-[10px] font-bold text-[#F7F7F7]/60 text-center py-8">Click cards in the pool to add them.</div>
            )}
          </div>
        </div>
      </div>
