...
export function MainMenu({ onNavigate, onHelp }: { onNavigate: (s: MetaScreen) => void; onHelp: () => void }) {
  ...
  const tiles: { key: MetaScreen; label: string; desc: string; icon: React.ReactNode; color: string; disabled?: boolean }[] = [
    { key: 'play', label: 'PLAY', desc: 'Battle the CPU', icon: <Swords className="w-8 h-8" />, color: 'bg-[#E53935] text-[#F7F7F7]' },
    { key: 'collection', label: 'COLLECTION', desc: guest ? 'Requires an account' : 'Browse your cards', icon: <Library className="w-8 h-8" />, color: 'bg-[#F7F7F7] text-[#1A1A1A]', disabled: guest },
    { key: 'decks', label: 'DECK BUILDER', desc: guest ? 'Requires an account' : 'Forge 30-card decks', icon: <Layers className="w-8 h-8" />, color: 'bg-[#FFD54F] text-[#1A1A1A]', disabled: guest },
    { key: 'store', label: 'STORE', desc: guest ? 'Requires an account' : 'Packs & cosmetics', icon: <Store className="w-8 h-8" />, color: 'bg-[#2C3E50] text-[#F7F7F7]', disabled: guest },
    { key: 'profile', label: 'PROFILE', desc: guest ? 'Requires an account' : 'Stats & customization', icon: <User className="w-8 h-8" />, color: 'bg-[#F7F7F7] text-[#1A1A1A]', disabled: guest },
  ];

  return (
    <div className="w-full min-h-screen bg-[#F7F7F7] text-[#1A1A1A] relative overflow-hidden">
      <div className="absolute inset-0 bg-[#FFD54F] pointer-events-none" style={{ clipPath: 'polygon(85% 0, 100% 0, 100% 100%, 70% 100%)' }} />
      ...
      <div className="relative z-10 flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-3">
          <div
            ...
            style={banner && !avatar ? { backgroundImage: `url(${banner.image_url})`, backgroundSize: 'cover' } : undefined}
          >
            {avatar?.image_url
              ? <img src={avatar.image_url} className="w-full h-full object-cover" />
              : <div className="w-full h-full flex items-center justify-center heading-font text-[#FFD54F] text-xl">{(profile?.username || 'G')[0].toUpperCase()}</div>}
          </div>
          <div>
            <div className="heading-font text-lg leading-none">{guest ? 'GUEST OPERATIVE' : profile?.username || '…'}</div>
            {profile && (
            ...
            {guest && <div className="text-[10px] font-bold text-[#2C3E50] mt-1">Progress is not saved in guest mode.</div>}
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={onHelp} className="btn-pop heading-font text-[11px] bg-[#FFD54F] text-[#1A1A1A] px-3 py-1.5 ink-border-sm shadow-hard-black-xs flex items-center gap-1">
            <BookOpen className="w-3.5 h-3.5" /> RULES
          </button>
          <button onClick={signOut} className="btn-pop heading-font text-[11px] bg-[#1A1A1A] text-[#F7F7F7] px-3 py-1.5 ink-border-sm shadow-hard-black-xs flex items-center gap-1">
      ...
      <div className="relative z-10 text-center mt-6 mb-10">
        ...
        <h1 className="text-5xl sm:text-7xl heading-font leading-none">
          SHIFTING<br /><span className="bg-[#1A1A1A] text-[#FFD54F] px-4 py-1 inline-block mt-2">MULTIVERSE TCG</span>
        </h1>
      </div>
