      ...
      <div className="max-w-4xl mx-auto p-5">
          ...
          <div className="absolute bottom-3 left-4 flex items-end gap-3">
            <div className="w-20 h-20 ink-border-md shadow-hard-black-xs bg-[#1A1A1A] overflow-hidden">
              {avatar?.image_url
                ...
                : <div className="w-full h-full flex items-center justify-center heading-font text-[#FFD54F] text-3xl">{profile.username[0]?.toUpperCase()}</div>}
            </div>
            <div className="pb-1">
              {editingName ? (
                <div className="flex gap-2 items-center">
                  <input value={nameDraft} onChange={(e) => setNameDraft(e.target.value)} maxLength={24}
                    className="px-2 py-1 bg-[#F7F7F7] ink-border-sm font-black heading-font text-sm" autoFocus />
                  <PopButton color="red" onClick={handleRename}><Check className="w-4 h-4" /></PopButton>
                </div>
              ) : (
                <button onClick={() => { setNameDraft(profile.username); setEditingName(true); }}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
          {[
            ...
            <div key={s.label} className="bg-[#F7F7F7] ink-border-sm shadow-hard-black-xs px-3 py-2 text-center">
        ...
        {error && <div className="mt-3"><Notice text={error} /></div>}
        ...
        {sections.map((sec) => {
          ...
          return (
            <div key={sec.type} className="mt-7">
              <h2 className="heading-font text-base mb-2 bg-[#1A1A1A] text-[#FFD54F] inline-block px-2 py-0.5">{sec.label}</h2>
              <div className="flex flex-wrap gap-3">
                {items.map((item) => {
                  const isEquipped = sec.equipped === item.id;
                  return (
                    <button key={item.id} onClick={() => handleEquip(item)}
                      className={cn(
                      ...
                      <div className={cn('overflow-hidden bg-[#2C3E50]',
                      ...
                      <div className="px-2 py-1 flex justify-between items-center gap-1">
                        ...
                        {isEquipped && <span className="text-[8px] font-black bg-[#E53935] text-[#F7F7F7] px-1 shrink-0">ON</span>}
                      </div>
                    </button>
                  );
                })}
                {items.length === 0 && (
                  <div className="text-[11px] font-bold text-[#2C3E50] py-3">Nothing owned yet — visit the Store.</div>
                )}
              </div>
            </div>
          );
