...
function shadowAttackers(state: GameState, pid: PlayerId): Set<string> {
  ...
  for (const u of candidates) {
    ...
    const survives =
      effMight(state, worst) < remainingGrit(state, u) || u.def.keywords?.includes('Unbreakable');
...
function runGame(deckA: DeckDef, deckB: DeckDef, seed: number, game: number): void {
  ...
  while (!state.winner && turns < MAX_TURNS) {
    ...
    const preOppMaxMight = preOppBiggestByIid.size ? Math.max(...preOppBiggestByIid.values()) : 0;
