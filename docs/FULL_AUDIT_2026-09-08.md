...
| 8 grading functions with mutable `search_path` | **Real.** `grading_base_fee`, `grading_bulk_mult`, `grading_grade_mult`, `grading_roll`, `grading_service_premium`, `grading_speed_mult`, `grading_turnaround`, `grading_voucher_fee` all had `proconfig = null`; the 2026-08-28 pass pinned the functions it touched and missed exactly these eight. | `alter function … set search_path to 'public'`, matching every other function in the schema. `grading_voucher_fee` also carried an explicit `anon` grant — the only named grant to the pre-sign-in role anywhere in the schema — revoked as well. |
